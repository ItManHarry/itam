...
from com.models import AuditLine, SysUser
...
def index():    
    ...
    if request.method == 'GET':
        ...
        try:
            code = session['line_view_search_code'] if session['line_view_search_code'] else ''  # 字典代码
            name = session['line_view_search_name'] if session['line_view_search_name'] else ''  # 字典名称
        except KeyError:
            code = ''
            name = ''
        form.code.data = code
...
def add():
    ...
    if form.validate_on_submit():
        line = AuditLine(id=uuid.uuid4().hex, code=form.code.data.upper(), name=form.name.data, bg_id=current_user.company_id, create_id=current_user.id)
        db.session.add(line)
        db.session.commit()
        flash('审批角色添加成功！')
