...
class ClazzForm(FlaskForm):
    ...
    def validate_code(self, field):
        if self.id.data is None or self.id.data == '':
            if BizAssetClass.query.filter_by(code=field.data.upper()).first():
                raise ValidationError('类别代码已存在!')
        else:
            old_code = BizAssetClass.query.get(self.id.data).code
            codes = (clazz.code for clazz in BizAssetClass.query.filter(BizAssetClass.code != old_code).all())
